...
from s4pi.data.utils import loadMapStack, str2bool, loadAIAMap, loadMap
from tqdm.contrib.concurrent import process_map
from itertools import repeat


def load_map_stack(euvi_stack):

    files_stack, calibration, euvi_resolution, euvi_reproject, aia_preprocessing = euvi_stack

    # calibration = 'aiapy'
    # euvi_resolution = 256
    # euvi_reproject = False
    euvi_stack = loadMapStack(files_stack, resolution=euvi_resolution, remove_nans=True,
                              map_reproject=euvi_reproject, aia_preprocessing=aia_preprocessing,
                              calibration=calibration)

    return np.asarray(euvi_stack)
...
if __name__ == "__main__":
    ...
    euvib_files = []
    euvib_columns = ['B/EUVI171', 'B/EUVI193', 'B/EUVI211', 'B/EUVI304']
    # TODO: FIX NAMES OF FILES (WRONG DISK)
    for index, row in tqdm(df.iterrows()):
        #print(row[euvib_columns])
        # euvib_files.append(row[euvib_columns].tolist())
        euvib_files.append([os.path.join('/mnt/disks/stereo_data/EUVI_prep',
                                         r.split('/mnt/disks/stereo_data/EUVI/')[-1]) for r in row[euvib_columns]])
        #breakpoint()
    # Stacks
    calibration = 'aiapy'
    euvi_resolution = 256
    ...
    euvib_img = process_map(load_map_stack, zip(euvib_files, repeat(calibration), repeat(euvi_resolution),
                                                repeat(euvi_reproject), repeat(aia_preprocessing)),
                            max_workers=4, chunksize=5, total=len(euvib_files))
    img_euvib = np.stack(euvib_img)
    euvib_min = np.mean(img_euvib, axis=(0, 2, 3), keepdims=True)
    euvib_max = np.std(img_euvib, axis=(0, 2, 3), keepdims=True)
    ...
    for l, wl_name in enumerate(wl_names):
        ...
        plt.draw()
        filename = os.path.join(result_path, f'forecast_aia2_{str(l)}_{wl_name.replace(" ", "")}.png')
