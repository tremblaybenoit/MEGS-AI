...
if __name__ == "__main__":
    ...
    wl_euvi_label = ['171Å', '195Å', '284Å', '304Å']
    ...
    aia_path = np.array(df['AIA_stack'])
    euvia_path = np.array(df['A/EUVI_stack'])
    ...
    for t in tqdm(range(t1, t2)):
        ...
        # EVE (reference)
        test_eve = (torch.stack([torch.tensor(eve_data[t])])).numpy()
        # SDO
        test_dates_aia = eve_dates[t]
        # test_aia = (torch.stack([torch.tensor(image) for image, eve in [data_module_aia.test_ds[t]]])).numpy()
        x = np.load(aia_path[t])
        test_aia = (torch.stack([torch.tensor(np.load(aia_path[t]))])).numpy()

        megs_aia = [irr for irr in tqdm(ipredict_uncertainty(model_aia, test_aia, return_images=True), total=len(test_aia))][0]
        # STEREO-A
        test_dates_euvia = eve_dates[t]
        test_euvia = (torch.stack([torch.tensor(np.load(euvia_path[t]).transpose(0, 2, 1))])).numpy()
        megs_euvia = [irr for irr in tqdm(ipredict_uncertainty(model_euvi, test_euvia, return_images=True), total=len(test_euvia))][0]
        # STEREO-B
        test_dates_euvib = eve_dates[t]
        test_euvib = (torch.stack([torch.tensor(np.load(euvib_path[t]).transpose(0, 2, 1))])).numpy()
        ...
        for col in range(ncols):
            if col == 0:
                ...
                ylabel_rotation = 90
                ylabel_pad = 5.0
                iti_note = [' (ITI-calibrated from 171Å)', ' (ITI-calibrated from 195Å)', 
                            ' (ITI-calibrated from 284Å)', ' (ITI-calibrated from 304Å)']
            elif col == 1:
            ...
            # Colorbar
            if t == t1:
                inputs_minmax.append([colorbar_minmax(megs_euvib[1][w]) for w in range(len(inputs))])
            ...
            for subrow in range(nsubrows):
                for subcol in range(nsubcols):
                    ...
                    ax.imshow(inputs[subrow*nsubcols+subcol], cmap=plt.get_cmap(cmaps[subrow*nsubcols+subcol]), norm='asinh', 
                              vmin=inputs_minmax[col][subrow*nsubcols+subcol][0], vmax=inputs_minmax[col][subrow*nsubcols+subcol][1])
                    ...
                    ax.text(0.98, 0.0, iti_note[subrow*nsubcols+subcol], horizontalalignment='right', verticalalignment='bottom', 
    ...
    for t in tqdm(range(t1, t2)):
        ...
        for col in range(ncols):
            if col == 0:
                ...
                inputs = megs_euvib[1]
                irr_mean, irr_err, irr_mi = megs_euvib[0]
                ylabel = r'Relative Spectral Irradiance (no units)'
                label_left = True
                label_right = False
                ylabel_position = "left"
                ylabel_rotation = 90
